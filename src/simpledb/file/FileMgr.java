...
public class FileMgr{
   ...
   //Allow to track block stats for write/red ops.
   private Map<String,IntermediateFileStats> blockStats = new HashMap<String,IntermediateFileStats>();  
   ...
   synchronized void read(Block blk, ByteBuffer bb) {
      try {
         ...
         this.updateBlockReadStats(blk, bb); //added for stats
      }
      catch (IOException e) {
   ...
   synchronized void write(Block blk, ByteBuffer bb) {
      try {
         ...
         this.updateBlockWrittenStats(blk, bb); //added for stats
      }
      catch (IOException e) {
   ...
   private FileChannel getFile(String filename) throws IOException {
      ...
      if(!this.blockStats.containsKey(filename)){ //aggiungo il file che voglio leggere alla traccia che mi serve.
		  this.blockStats.put(filename, new IntermediateFileStats());
	   }
      return fc;
   }
   
   /* Stat's related procedures */
...
//	   String readLog = "["+timestamp+"]: "+ "<"+b.fileName()+","+b.number()+">" +":"+bb.toString();
	   ...
	   this.blockStats.put(b.fileName(), temp);
   }

   //TODO time
   private void updateBlockWrittenStats(Block b, ByteBuffer bb){
	   String timestamp = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss.SSS").format(new Date());
	   String writeLog = "["+timestamp+"]: "+ "<"+b.fileName()+","+b.number()+">" +": ["+bb.position()+", "+bb.remaining()+"]";
//	   String writeLog = "["+timestamp+"]: "+ "<"+b.fileName()+","+b.number()+">" +":"+bb.toString(); 
	   IntermediateFileStats temp = this.blockStats.get(b.fileName());
	   temp.blkWrittenInc(); //incrementa il numero di blocchi letti per il file
	   temp.putWrittenLog(writeLog);//inserisci il log relativo!
	   this.blockStats.put(b.fileName(), temp); //aggiorna la mappa :D
   }
   
   public final Map<String,IntermediateFileStats> getMapStats(){
